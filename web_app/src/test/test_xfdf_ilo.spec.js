...
test('test prohibition number, without the prefix, shown on ilo copy', () => {
...
test('test driver surname is redacted from ilo copy', () => {
    store.commit("updateFormField", { "target": { "value": "Smith", "id": "last_name"}})
    const actual = store.getters.getXfdfMappings(state.currently_editing_form_object, document_type)
    expect(actual['DRIVER_SURNAME']).toEqual("--")
})

test('test driver first name is redacted from ilo copy', () => {
    store.commit("updateFormField", { "target": { "value": "Brian", "id": "first_name"}})
    const actual = store.getters.getXfdfMappings(state.currently_editing_form_object, document_type)
    expect(actual['DRIVER_GIVEN']).toEqual("--")
})

test('test driver licence number is redacted from ilo copy', () => {
    store.commit("updateFormField", { "target": { "value": "5123456", "id": "drivers_number"}})
    const actual = store.getters.getXfdfMappings(state.currently_editing_form_object, document_type)
    expect(actual['DRIVER_DL_NUMBER']).toEqual("--")
})

test('test driver province / state is redacted from ilo copy', () => {
    store.commit("updateFormField", { "target": { "value": "5123456", "id": "province"}})
    const actual = store.getters.getXfdfMappings(state.currently_editing_form_object, document_type)
    expect(actual['DRIVER_DL_PROVINCE']).toEqual("--")
})

test('test driver date of birth is redacted from ilo copy', () => {
    store.commit("updateFormField", { "target": { "value": "5123456", "id": "dob"}})
    const actual = store.getters.getXfdfMappings(state.currently_editing_form_object, document_type)
...
test('test prohibition reason alcohol is shown on ilo copy', () => {
...
test('test prohibition reason drugs is shown on ilo copy', () => {
...
test('test the date and time of care and control is shown on ilo copy', () => {
...
test('test that the selected impound lot operator is shown on ilo copy', () => {
...
test('test when vehicle impounded the status is shown on ilo copy', () => {
...
test('test when vehicle NOT impounded the status is shown on ilo copy', () => {
...
test('test location of vehicle keys is shown on ilo copy', () => {
